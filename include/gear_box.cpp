 ...
 void gear_input(std::vector<gear_box>&v_gears)
{
       ...
       for (int i = 0; i < v_gears.size(); i++)
    {
        ...
        for ( int i=0;i<10;i++)
                ...
                {
                A.gearbox_model =default_model[i];
                A.gear_ratio=default_ratio[i];
                A.efficiency=default_eff[i]/100.0;
                A.gear_mass=default_mass[i];
                A.gear_diameter=default_diameter[i];
                v_gears.push_back(A);
                }
